...
def train(test_envs, args, hparams, n_steps, use_alpha, all_layers, checkpoint_freq, logger, writer, target_env=None):
    logger.info("")
    ...
    # done
    change_rate_history = algorithm.change_rate_history
    change_percentage_history = algorithm.change_percentage_history
    
    change_rate_path = os.path.join(args.out_dir) + "/change_rate_history.pkl"
    change_percentage_path = os.path.join(args.out_dir) + "/change_percentage_history.pkl"
    
    with open(change_rate_path, 'wb') as file:
        pickle.dump(change_rate_history, file)
    with open(change_percentage_path, 'wb') as file:
        pickle.dump(change_percentage_history, file)
