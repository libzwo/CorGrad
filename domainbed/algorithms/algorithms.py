...
from domainbed.gradient import get_method, METHODS
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, n_steps, use_alpha):
...
class GradSurgery(Algorithm):

    def __init__(self, input_shape, num_classes, num_domains, hparams, n_step, use_alpha, all_layers):
        super(GradSurgery, self).__init__(
            input_shape, num_classes, num_domains, hparams)
        self.grad_fn = get_method(hparams['method'])
        self.n_parts = hparams['n_parts']
        self.alpha = hparams['alpha']
        self.n_step = n_step
        self.use_alpha = use_alpha
        self.all_layers = all_layers
        # self.network = networks.AlexNet(num_classes=num_classes, use_pretrained=True, hparams=hparams)
        ...
        self.change_rate_history = [] ### new
        self.change_percentage_history =[] ###new
    ...
    def update(self, x, y, **kwargs):
        ...
        domain_grads = []
        train_loss = 0.0
        for (images, target) in zip(all_x, all_y):
            ...
            loss.backward()
            if self.all_layers:
                domain_grads.append(self.network.get_all_grads())
            else:
                domain_grads.append(self.network.get_conv_grads())
            # domain_grads.append(self.network.get_grads())
        train_loss /= len(all_x)
        if self.use_alpha:
            alpha = AlphaSheduler(1.0, self.n_step, **kwargs)
            a = alpha.alp()
        new_grads, change_rate, change_percentage_mean = self.grad_fn(domain_grads, self.n_parts, self.alpha)# modify gradients according to grad_fn
        # new_grads, change_rate = self.grad_fn(domain_grads, self.n_parts, self.alpha)# modify gradients according to grad_fn
        if self.all_layers:
            self.network.set_all_grads(new_grads)
        else:
            self.network.set_conv_grads(new_grads)
        # self.network.set_grads(new_grads)
        self.change_rate_history.append(change_rate) ### new
        self.change_percentage_history.append(change_percentage_mean) ###new
...
class AlphaSheduler(nn.Module):
    def __init__(self, alpha=1.0, max_iter=5000, **kwargs):
        super(AlphaSheduler, self).__init__()
        self.alpha = alpha
        self.max_iter = max_iter
        self.curr_iter = kwargs["step"]

    def alp(self):
        a = 1. - np.log(self.curr_iter + 1) / np.log(self.max_iter)
        return a
