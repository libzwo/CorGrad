...
def main():
    ...
    parser.add_argument("configs", nargs="*")##可变长参数
    ...
    parser.add_argument("--use_alpha", action="store_true", help="change alpha with time")
    parser.add_argument("--all_layers", action="store_true", help="change all layers or conv params")

    args, left_argv = parser.parse_known_args()

    # setup hparams
    hparams = hparams_registry.default_hparams(args.algorithm, args.dataset)
    ##hparams_registry是一个超参数注册表，存储了所有支持算法和数据集的默认超参数，调用default_hpaeams方法传递算法和数据集名称
